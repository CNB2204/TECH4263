...
string[] usCities = { "Memphis", "West Memphis", "Phoenix", "Eugene", "Seattle" };
...
foreach (var usCity in usCities)
{
    Console.WriteLine($"Looking up: {usCity}...");

    // ── STEP 1: Geocoding API — city name → coordinates ────────────
    try
    {
        var geoUrl = $"https://geocoding-api.open-meteo.com/v1/search?name={usCity}&count=1";
        var geoResponse = await httpClient.GetAsync(geoUrl);
        ...
        if (results.GetArrayLength() == 0)
        {
            Console.WriteLine($"  [!] City '{usCity}' not found.\n");
            continue;
        }
